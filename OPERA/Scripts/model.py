...
def LIN_Block(h_dim, nLin):
    torch.manual_seed(0)
    return ModuleList([Sequential(Linear(h_dim, h_dim), ReLU(), Linear(h_dim, h_dim), ReLU()) for _ in range(nLin)]).to(device)
...
class HybridGICN(MessagePassing):
    def __init__(self, trial, props):
        ...
        #self.LIN_List = [_ for _ in range(self.props)]
        
        
        for l in range(self.props):
            self.GCN_List[l] = GCN_Block(h_dim, trial.suggest_int(f'nGCN{l+1}', 1,3))
            #self.LIN_List[l] = LIN_Block(h_dim, trial.suggest_int(f'nLIN{l+1}', 1,3))
    ...
    def forward(self, x, edge_index, edge_attr, batch):
        ...
        for npr in range(self.props):
            ...
            # for m in range(len(self.LIN_List[npr])):
            #     inter_val[f'x{npr}'] = self.LIN_List[npr][m](inter_val[f'x{npr}'])
